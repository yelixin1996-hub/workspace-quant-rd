# -*- coding: utf-8 -*-
"""
多因子Alpha量化交易系统 v12.0 (时间止损版)
===============================================
核心发现(v11):
- 持有16+天: 平均-1.79%，胜率38% ❌ 亏损根源
- 持有1-15天: 平均+23%~+34%，胜率100% ✅
- 到期卖出: 24笔, 平均-1.43%, 胜率42%
- 止损(已移除): 15笔, 平均-11.08%, 0%胜率
- 止盈: 6笔, 平均+30.15%, 100%胜率

v12改进:
1. 时间止损(核心): 持有>=N天且浮亏<0时强制卖出，截断长持亏损
2. 止盈保持: 25%止盈不变
3. 到期保持: 20天到期不变
4. RSI辅助增强: RSI>85且浮盈>5%时止盈(v11为88)
5. 负面清单强化: RSI>80直接不买入(v11仅减分)
6. 参数扫描: time_stop_days (8/10/12/15)
"""
...
class Selector:
    ...
    def select_top(self, date, market_env='neutral', min_score=90, top_n=3):
        ...
        for code, df in self.stocks.items():
            ...
            # 负面清单 (RSI>80已在上方continue，这里处理其他负面因素)
            if mom < -0.15:
                score -= 30
...
def run_backtest(stocks, start_date='2024-01-01', end_date='2026-03-20', 
                 ...
                 max_hold=20, min_score=90, time_stop_days=12):
    """v12: 时间止损版 — 持有>=N天且浮亏时强制卖出"""
    ...
    log(f"  参数: 止盈{take_profit*100:.0f}% | 时间止损{time_stop_days}天 | 到期{max_hold}天 | 最低分{min_score}")
    ...
    for i, date in enumerate(dates):
        ...
        if position:
            ...
            # 2. [v12] RSI超买辅助止盈 (RSI>85且浮盈>5%)
            elif today_rsi > 85 and pnl > 0.05:
                sell_reason = 'RSI超买'
            
            # 3. [v12核心] 时间止损: 持有>=N天且浮亏，强制卖出
            elif hold_days >= time_stop_days and pnl < 0:
                sell_reason = '时间止损'
...
def quick_scan(stocks, date_range=['2024-01-01', '2024-06-01']):
    ...
    time_stop_options = [8, 10, 12, 15]
    take_profit_options = [0.20, 0.25, 0.30]
    max_hold_options = [18, 20, 25]
    
    total_combos = len(time_stop_options) * len(take_profit_options) * len(max_hold_options)
    combo_idx = 0
    
    for time_stop in time_stop_options:
        for take_profit in take_profit_options:
            for max_hold in max_hold_options:
                if time_stop >= max_hold:
                    continue
                ...
                if annual > best_annual:
                    ...
                    log(f"  *** 新最佳: 年化{annual:.2f}% | 时间止损{time_stop}天 止盈{take_profit*100:.0f}% 持有{max_hold}天 | 胜率{win_rate:.0f}% 平均{avg_pnl:.2f}%")
    
    log(f"\n  扫描完成，共{len(all_results)}种参数组合")
    
    log("\n  === 按时间止损天数汇总 ===")
    for ts in time_stop_options:
        group = [r for r in all_results if r['time_stop'] == ts]
        if group:
            best_in_group = max(group, key=lambda x: x['annual'])
            avg_annual = np.mean([r['annual'] for r in group])
            log(f"    时间止损{ts}天: 平均年化{avg_annual:.2f}%, 最佳年化{best_in_group['annual']:.2f}% "
                f"(止盈{best_in_group['take_profit']*100:.0f}% 持有{best_in_group['max_hold']}天)")
...
if __name__ == '__main__':
    log("=" * 60)
    log("多因子Alpha量化交易系统 v12.0 (时间止损版)")
    log("=" * 60)
