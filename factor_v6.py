# -*- coding: utf-8 -*-
"""
多因子Alpha量化交易系统 v6.0 (知识库因子优化版)
==============================================
基于因子知识库和v5回测的改进:
1. 熔断放宽 - 连亏5次才触发
2. 去掉均线死叉强制卖出 - v5显示亏损严重
3. 持有偏好 - 偏向2-3天和4-7天
4. 新因子加权 - RSI/Amihud参与选股
5. 扩大选股池 - 200→400只
"""
...
def load_all_stocks(data_dir='E:/data', min_rows=200, max_stocks=400):
    """加载所有股票"""
    stocks = {}
...
def add_factors(df):
    ...
    df['amihud_ratio'] = df['amihud'] / (df['amihud_20'] + 1e-8)
...
class Selector:
    ...
    def select_top(self, date, market_env='neutral', top_n=5):
        """选择最佳股票"""
        candidates = []
        ...
        for code, df in self.stocks.items():
            ...
            if score > 0:
                candidates.append((code, score, row['close'], reasons, rsi, pos))
...
def run_backtest(stocks, start_date='2024-01-01', end_date='2026-03-20', 
    ...
    for i, date in enumerate(dates):
        ...
        # 持仓检查
        if position:
            code, entry_price, entry_date, shares, entry_rsi, entry_pos = position
        ...
        if position is None and not fuse_active:
            top_stocks = selector.select_top(date, market_env=market_env, top_n=5)
            if top_stocks:
                code, score, price, reasons, rsi, pos = top_stocks[0]
                if price * 100 * 1.0003 <= cash:
                    shares = 100
                    cost = price * shares * 1.0003
                    cash -= cost
                    position = (code, price, date, shares, rsi, pos)
                    highest_price = price
                    trades.append({
        ...
        # 记录权益
        if position:
            code, entry_price, _, shares, _, _ = position
            df = stocks.get(code)
            today_row = df[df['date'] == date]
...
def quick_scan(stocks, date_range=['2024-01-01', '2024-06-01']):
    ...
    for stop_loss, take_profit, max_hold, trailing_stop in params:
        ...
        if annual > best_annual:
            ...
            log(f"*** 新最佳: 年化{annual:.2f}% 止损{stop_loss} 止盈{take_profit} 持有{max_hold}")
...
if __name__ == '__main__':
    log("=" * 60)
    log("多因子Alpha量化交易系统 v6.0")
    log("=" * 60)
    
    log("\n[1] 加载股票数据...")
    stocks = load_all_stocks('E:/data', min_rows=250, max_stocks=400)
